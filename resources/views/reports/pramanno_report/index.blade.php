...
<section class="content-header">
    <div aria-label="breadcrumb" class="card-breadcrumb">
        <h5><a href="{{ url('/dashboard') }}"  style="text-decoration: none; color: black;">{{ __('messages.dashboard') }}</a> > {{ __('প্রামান্য চিত্রের রিপোর্ট') }} </h5>
    </div>
...
<div class="card-body table-responsive">
    ...
    <form id="filmReportFilter" class="row g-3 mb-4 align-items-end" action="{{ route('reports.export', ['type' => 'docufilm']) }}" method="POST">
    @csrf
        <div class="col-md-2">
    ...
    <script>
        $(document).ready(function () {
            $('#showReport').on('click', function () {
                ...
                $.ajax({
                    url: "{{ route('reports.showPramannoReport') }}",
                    method: "POST",
                    type: "POST",
                    data: {
                        from_date: from_date,
